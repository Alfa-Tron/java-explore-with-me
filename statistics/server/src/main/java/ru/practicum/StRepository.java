...
public interface StRepository extends JpaRepository<EndpointHit, Long> {
    @Query("SELECT new ru.practicum.ViewStatsDto(h.app, h.uri, COUNT(DISTINCT h.ip)) FROM EndpointHit AS h WHERE h.timestamp BETWEEN :start AND :end AND h.uri in :uris GROUP BY h.app, h.uri ORDER BY COUNT(h.ip) DESC")
